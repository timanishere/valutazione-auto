# Import DAg and task from Airflow. DAG is how to define pipeline. Task is how to define each individual tasks inside the pipeline
from airflow.decorators import dag, task

# Import pendulum to define start/end times in airflow
import pendulum
...
@dag(
   # give dag a name
   dag_id = 'audi_pipeline',


   # tell airflow to run this pipeline daily
...
# define function which contains logic for data pipeline
def audi_pipeline():
  
   # write task to get episodes
   @task()
   def audi_data_ingestion():
      # EXTRACT DATA: STARTS
      ...
      cursor.execute('SELECT make,model,autoscout_url_make, autoscout_url_model FROM audi_make_and_model_tbl')
...
# Need to end this with a call to our function 
audi_pipeline_output = audi_pipeline()